  ...
  return (
    <div>
      <h2 id="reservation-heading" data-cy="ReservationHeading">
        <Translate contentKey="slappApp.reservation.home.title">Reservations</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="slappApp.reservation.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/reservation/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="slappApp.reservation.home.createLabel">Create new Reservation</Translate>
          </Link>
        </div>
      </h2>
        ...
        {reservationList && reservationList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="slappApp.reservation.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="slappApp.reservation.customer">Customer</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="slappApp.reservation.room">Room</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {reservation.startDateTime ? (
                      <TextFormat type="date" value={reservation.startDateTime} format={APP_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>
                    {reservation.endDateTime ? <TextFormat type="date" value={reservation.endDateTime} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>{reservation.totalPrice}</td>
                  <td>
                    <Translate contentKey={`slappApp.ReservationStatus.${reservation.status}`} />
                  </td>
                  ...
                  <td>
                    {reservation.cancelledAt ? <TextFormat type="date" value={reservation.cancelledAt} format={APP_DATE_FORMAT} /> : null}
                  </td>
                  <td>{reservation.cancelReason}</td>
                  <td>
                    {reservation.customer ? <Link to={`/user-profile/${reservation.customer.id}`}>{reservation.customer.id}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="slappApp.reservation.home.notFound">No Reservations found</Translate>
            </div>
          )
        )}
      </div>
