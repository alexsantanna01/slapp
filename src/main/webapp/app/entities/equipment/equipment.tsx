  ...
  return (
    <div>
      <h2 id="equipment-heading" data-cy="EquipmentHeading">
        <Translate contentKey="slappApp.equipment.home.title">Equipment</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="slappApp.equipment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/equipment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="slappApp.equipment.home.createLabel">Create new Equipment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {equipmentList && equipmentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="slappApp.equipment.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('name')}>
                  <Translate contentKey="slappApp.equipment.name">Name</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('name')} />
                </th>
                ...
                <th className="hand" onClick={sort('equipmentType')}>
                  <Translate contentKey="slappApp.equipment.equipmentType">Equipment Type</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('equipmentType')} />
                </th>
                ...
                <th>
                  <Translate contentKey="slappApp.equipment.room">Room</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {equipmentList.map((equipment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{equipment.available ? 'true' : 'false'}</td>
                  <td>
                    <Translate contentKey={`slappApp.EquipmentType.${equipment.equipmentType}`} />
                  </td>
                  <td>{equipment.createdAt ? <TextFormat type="date" value={equipment.createdAt} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{equipment.updatedAt ? <TextFormat type="date" value={equipment.updatedAt} format={APP_DATE_FORMAT} /> : null}</td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/equipment/${equipment.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/equipment/${equipment.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="slappApp.equipment.home.notFound">No Equipment found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={equipmentList && equipmentList.length > 0 ? '' : 'd-none'}>
